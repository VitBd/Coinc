...
import { PortfolioBalanceCard } from '@/components/concept/portfolio-balance-card';
import { SummaryCard } from '@/components/dashboard/summary-card';
import { PortfolioCardV2, type PortfolioCardProps } from '@/components/dashboard/portfolio-card-v2';
import { useToast } from "@/hooks/use-toast";
import { DailyEarnAccount } from '@/components/concept/daily-earn-account';
import { CashAccount } from '@/components/concept/cash-account';
...
const allAccountsData = {
  ...
  chartData: Array.from({ length: 30 }, (_, i) => ({ name: `Day ${i + 1}`, value: 40000 + Math.random() * 5000 + i * 50 })),
        ...
        <div className="grid gap-4 md:gap-8 lg:grid-cols-3">
          {portfolioCardsData.map((portfolio) => (
            <PortfolioCardV2
              key={portfolio.title}
              {...portfolio}
              isOwned={portfolio.isOwned !== false}
